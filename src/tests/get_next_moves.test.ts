...
describe('get_next_moves', () => {
	test('variations', () => {
		...
		// expect(get_next_move(null, e5_node)).toBeNull();
		expect(get_next_moves(root)).toStrictEqual([e5_node, d5_node]);
		expect(get_next_moves(e5_node)).toStrictEqual([]);
		expect(get_next_moves(d5_node)).toStrictEqual([]);
		expect(get_next_moves(d4_node)).toStrictEqual([Nf6_node]);
		expect(get_next_moves(Nf6_node)).toStrictEqual([]);
		expect(get_next_moves(Nf3_node)).toStrictEqual([b6_node]);
		expect(get_next_moves(b6_node)).toStrictEqual([]);
	});
});
