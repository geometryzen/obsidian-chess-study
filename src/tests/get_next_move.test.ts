...
describe('get_next_move', () => {
	test('variations', () => {
		...
		expect(get_next_move(root)).toBe(e5_node);
		expect(get_next_move(e5_node)).toBeNull();
		expect(get_next_move(d5_node)).toBeNull();
		expect(get_next_move(d4_node)).toBe(Nf6_node);
		expect(get_next_move(Nf6_node)).toBeNull();
		expect(get_next_move(Nf3_node)).toBe(b6_node);
		expect(get_next_move(b6_node)).toBeNull();
	});
});
