...
import { bfsGeneratorLR } from '../lib/tree/bfsGeneratorLR';
...
describe('bfsGenerator', () => {
	test('1. e4', () => {
		...
		const sans: string[] = [];
		const bfsIterator = bfsGeneratorLR(e4_node);
	...
	test('variations', () => {
		...
		expect(sans.length).toBe(6);
		expect(sans[0]).toBe('e4');
		expect(sans[1]).toBe('e5');
		expect(sans[2]).toBe('d4');
		expect(sans[3]).toBe('d5');
		expect(sans[4]).toBe('Nf6');
		expect(sans[5]).toBe('Nf3');
	});
});
