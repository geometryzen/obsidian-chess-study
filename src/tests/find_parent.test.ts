...
describe('find_parent', () => {
	test('variations', () => {
		...
		expect(find_parent(null, e5_node)).toBeNull();
		expect(find_parent(root, root)).toBeNull();
		expect(find_parent(root, e5_node)).toBe(e4_node);
		expect(find_parent(root, d5_node)).toBe(e5_node);
		expect(find_parent(root, d4_node)).toBe(root);
		expect(find_parent(root, Nf6_node)).toBe(d4_node);
		expect(find_parent(root, Nf3_node)).toBe(d4_node);
		expect(find_parent(root, b6_node)).toBe(Nf3_node);
	});
});
