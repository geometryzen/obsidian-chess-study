...
export function model_from_json(fileContent: ChessStudyFileContent): {
	model: ChessStudyModel;
	version: string;
} {
	const comment = fileContent.comment;
	const headers = fileContent.headers;
	const root: ChessStudyNode | null = node_from_moves(fileContent.moves);
...
function node_from_moves(ms: ChessStudyFileMove[]): ChessStudyNode | null {
	...
	if (Array.isArray(moves)) {
		...
		while (moves.length > 0) {
			const move = moves.pop() as ChessStudyFileMove;
			let right: ChessStudyNode | null = null;
			const variantions = [...move.variants];
			while (variantions.length > 0) {
				const variation = variantions.pop() as ChessStudyFileVariation;
				...
				right = node_from_moves(variation.moves);
			}
